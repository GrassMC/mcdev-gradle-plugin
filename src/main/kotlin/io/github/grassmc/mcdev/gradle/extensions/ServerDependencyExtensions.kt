...
private object ServerDependencies {
    const val R0_1_SUFFIX = "-R0.1"

    val SPIGOT_API = DependencyHolder("org.spigotmc", "spigot-api", true, R0_1_SUFFIX)

    // For version 1.17 or later
    val PAPER_API = DependencyHolder("io.papermc.paper", "paper-api", true, R0_1_SUFFIX)

    // For version 1.16.5 or older
    val PAPER_API_LEGACY = DependencyHolder("com.destroystokyo.paper", "paper-api", true, R0_1_SUFFIX)

    // For version 1.8.8
    val PAPER_API_1_8_8 = DependencyHolder("org.github.paperspigot", "paperspigot-api", true, R0_1_SUFFIX)

    val PURPUR_API = DependencyHolder("org.purpurmc.purpur", "purpur-api", true, R0_1_SUFFIX)
}
...
fun DependencyHandler.spigotApi(version: MinecraftVersion): String {
    ...
    return ServerDependencies.SPIGOT_API.notation(version.asString())
}

fun DependencyHandler.spigotApi(version: String) = spigotApi(MinecraftVersion.matching(version))

fun DependencyHandler.paperApi(version: MinecraftVersion): String {
    val dependency = when (version) {
        ...
        V1_8_8 -> ServerDependencies.PAPER_API_1_8_8
        in V1_11..V1_16_5 -> ServerDependencies.PAPER_API_LEGACY
        else -> ServerDependencies.PAPER_API
    }
...
fun DependencyHandler.purpurApi(version: MinecraftVersion): String {
    check(version <= V1_16_5) { "purpur-api doesn't support versions older than Minecraft 1.16.5!" }
    return ServerDependencies.PURPUR_API.notation(version.asString())
}
